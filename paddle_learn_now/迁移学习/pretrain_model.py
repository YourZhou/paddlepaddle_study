...
# 通过使用PaddlePaddle的接口得到的flowers数据集的
# 图片是3通道宽高都是224的彩色图，总类别是102种
# 定义输入层.
...
# 要重新修改分类器。这个也是训练新模型的最大不同点，
# 通过分离分类器来解决两个数据集的不同类别的问题。
...
# 通过if_exist函数判断网络所需的模型文件是否存在，
# 然后再通过调用fluid.io.load_vars加载存在的模型文件。
# 要留意的是这里使用的是之前克隆的基本程序。
# 官方提供的原预训练模型
...
fluid.io.load_vars(executor=exe,
                   ...
                   predicate=if_exist,
                   main_program=base_model_program)
