...
# 定义数据输入层，我们使用的是图片大小是224，
# resize_size是用于统一缩放到这个大小，
# 然后再随机裁剪成crop_size大小，crop_size才是最终训练图片的大小。
crop_size = 224
resize_size = 250
...
# 接着获取MobileNet网络的分类器，
# 传入的第一个参数就是上面定义的输入层，第二个是分类的类别大小
model = mobilenet_v1.net(image, 2)

# 获取损失函数和平均准确率函数，还有测试程序和优化方法，
# 这个优化方法我加了正则，因为爬取的图片数量太少，
# 在训练容易过拟合，所以加上正则一定程度上可以抑制过拟合
# 获取损失函数和准确率函数
...
# 获取训练测试是所以想的数据读取reader，
# 通过使用paddle.batch()函数可以把多条数据打包成一个批次，
# 训练的时候是按照一个个批次训练的。
# 获取自定义数据
